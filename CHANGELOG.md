...
### Security
- **Heading parsing took time cubic in a line's length.** The heading patterns in `buildSectionCharMap` and `findSectionForText`, which `refract analyze` runs on every revision, and in `extractHeadingMap` (behind `deriveSectionHeading`) let the spaces on a line be split between three quantifiers. One line of `=` and a few thousand spaces took minutes to parse (four at 8,000 characters), so a single vandalized or crafted revision could stall an analysis. All three now run in linear time, with the same results on every single-line heading; `extractHeadingMap` no longer joins a heading across two lines, which the old pattern did because `\s` matched newlines.

...
| Package | In this tree | On npm (2026-09-24) | Downstream consumers |
|---|---|---|---|
| `@refract-org/evidence-graph` | 0.5.1 | 0.5.0 | every package; NextConsensus; refract-ui (mirrored types) |
| `@refract-org/ingestion` | 0.3.2 | 0.3.1 | mcp, cli; NextConsensus |
| `@refract-org/analyzers` | 0.5.1 | 0.5.0 | mcp, cli; NextConsensus |
| `@refract-org/mcp` | 0.1.0 | not published | cli |
| `@refract-org/eval` | 0.2.2 | 0.2.1 | cli (optional) |
| `@refract-org/cli` | 0.5.15 | 0.5.7 — cannot start | refract-py, Docker image, operators |
...
that no published ingestion has. Until the next release reaches npm, run the CLI
from a source checkout. The three library packages on npm install and import
normally.
